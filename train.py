...
def get_model(arch, hidden_units, gpu):
...
def main():
    ...
    dataloaders = [torch.utils.data.DataLoader(image_datasets[0], batch_size=15, shuffle=True),
                   torch.utils.data.DataLoader(image_datasets[1], batch_size=15, shuffle=True),
                   torch.utils.data.DataLoader(image_datasets[2], batch_size=15, shuffle=True)]
    ...
    validloader = torch.utils.data.DataLoader(valid_data, batch_size=15, shuffle=True)
    testloader = torch.utils.data.DataLoader(test_data, batch_size=15, shuffle=True)

    with open('cat_to_name.json', 'r') as f:
        cat_to_name = json.load(f)

    model, device, feat = get_model(args.arch, args.hidden_units, args.gpu)
